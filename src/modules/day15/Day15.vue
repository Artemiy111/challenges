<template>
  <div class="view">
    ...
    <div class="wrapper">
      ...
      <p></p>
      <ul class="plates">
        <li v-if="tacoItems.length === 0">Loading Tapas...</li>
        <li v-for="(tacoItem, index) in tacoItems" :key="index">
          <input
            type="checkbox"
            :checked="tacoItem.checked"
            :id="`item${index}`"
            @change="toggleChecked(index)"
          />
          <label :for="`item${index}`">{{ tacoItem.text }}</label>
        </li>
      </ul>
      <form class="add-items" @submit.prevent="addItem(inputText)">
        <input type="text" name="item" v-model="inputText" placeholder="Item Name" required />
...
const loadFromLocalStorage = () => {
  const tacoItemsFromLocalStorage = JSON.parse(
    localStorage.getItem('tacoItems') || '[]'
  ) as TacoItem[]
  tacoItems.value = tacoItemsFromLocalStorage
}

const safeToToLocalStorage = () => {
  console.log(tacoItems.value)
  localStorage.setItem('tacoItems', JSON.stringify(tacoItems.value))
}
watch(tacoItems, safeToToLocalStorage, { deep: true })

const addItem = (text: string) => {
  tacoItems.value.push({
    text,
    checked: false,
  })
}
...
const toggleChecked = (index: number) => {
  tacoItems.value[index].checked = !tacoItems.value[index].checked
}
</script>
