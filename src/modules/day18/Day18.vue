...
const allMinutes = ref(0)
const allSeconds = ref(0)

onMounted(() => {
  const items = Array.from(document.querySelectorAll('[data-time]')) as HTMLLIElement[]
  for (const item of items) {
    const timeString = item.getAttribute('data-time')
    const [minutes, seconds] = timeString.split(':').map(s => parseInt(s))
    allSeconds.value += seconds
    allSeconds.value += minutes * 60
  }
  allMinutes.value = Math.trunc(allSeconds.value / 60)
  allSeconds.value %= 60
})
</script>
